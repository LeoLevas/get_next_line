...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   u_get_next_line_utils_bonus.c                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: llevasse <llevasse@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/14 10:58:13 by llevasse          #+#    #+#             */
/*   Updated: 2023/06/07 12:41:08 by llevasse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "u_get_next_line_bonus.h"
...
unsigned char	*ft_strjoin(unsigned char *s1, unsigned char *s2)
{
	...
	if (!s1 || !s2)
		return (NULL);
	joined = malloc((ft_strlen(s1) + ft_strlen(s2)) * sizeof(unsigned char) + 1);
	if (!joined)
		return (free(s1), NULL);
	while (s1[i] != '\0')
		joined[j++] = s1[i++];
